...
Profile: Address Book (F-29) is QA. This user's Order History (F-30) is QA in localStorage. Admin also has Address Book on the profile menu (ADM-080). Company offices are created on Manage Addresses (F-14), not on Address Book.
...
F-30 — User Order History: this user's past orders. Profile menu. Stored in localStorage. Not admin Order History (F-16). Tickets: USR-090–USR-098.
...
F-20 — Remove the footer English/Français dropdown. Language belongs in the **navbar only** (`EN` / `FR` on `AppHeader.vue`). Footer has no language control. Designer may still show a duplicate.
